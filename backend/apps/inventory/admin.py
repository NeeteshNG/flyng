...
class StorageLocationAdmin(ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('zone', 'code', 'location_type')
        }),
        (_('Position'), {
        ...
        (_('Capacity'), {
            'fields': ('max_bins', 'is_full')
        }),
        (_('Status'), {
            'fields': ('is_accessible', 'is_active')
        }),
        (_('Notes'), {
            'fields': ('notes',),
            'classes': ('collapse',),
...
class BinLabelTypeAdmin(ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('organization', 'name', 'label_type', 'is_active')
        }),
        (_('ArUco Settings'), {
...
class StorageBinTemplateAdmin(ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('organization', 'name', 'description', 'label_type', 'is_active')
        }),
        (_('Dimensions'), {
            'fields': ('width', 'height', 'depth', 'volume_display')
        }),
        (_('Capacity'), {
            'fields': ('max_weight_kg', 'coordinate_type')
        }),
        (_('Configuration'), {
            'fields': ('config',),
            'classes': ('collapse',),
...
class StorageBinAdmin(ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('location', 'template', 'code', 'label_value')
        }),
        (_('Position'), {
            'fields': ('position_index',)
        }),
        (_('Status'), {
            'fields': ('current_weight_kg', 'item_count', 'is_full', 'is_active')
        }),
        (_('Notes'), {
            'fields': ('notes',),
            'classes': ('collapse',),
    ...
    def weight_status(self, obj):
        ...
        return format_html(
            '<span style="color: {};">{:.1f}% ({:.1f} kg)</span>',
            color,
            pct,
            obj.current_weight_kg
        )
...
class ItemCategoryAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Hierarchy'), {
            'fields': ('parent', 'level', 'full_path_display')
        }),
        (_('Display'), {
...
class InventoryItemAdmin(ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('organization', 'sku', 'name', 'category', 'is_active')
        }),
        (_('Description'), {
